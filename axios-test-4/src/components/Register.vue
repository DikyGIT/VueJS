<template>
  <div class="registrasi">
    <div class="registrasi-box">
      ...
      <b-form @submit="onRegis" v-if="show">
        <b-form-group class="registrasi-label" id="input-group-1" label="Email address" label-for="input-1">
          <b-form-input id="input-1" v-model="form.email" type="email" placeholder="Enter Email" required></b-form-input>
        </b-form-group>

        <b-form-group class="registrasi-label" id="input-group-2" label="Your Password" label-for="input-2">
          <b-form-input id="input-2" v-model="form.password" type="password" placeholder="Enter Password" required></b-form-input>
        </b-form-group>

        <b-button class="button-registrasi" @click="registration()" variant="primary">Sign Up</b-button>
        <p class="login-label">Already have an Account? <a href="/">Sign In</a></p>
      </b-form>
    </div>
  </div>
</template>
